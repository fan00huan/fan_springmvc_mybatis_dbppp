...
`id` int  not null comment '記録号',
`useid` varchar(20) not null comment '员工编号',
`work_start` datetime  null comment '上班时间',
`work_end` datetime null comment '下班时间',
`work_rest` varchar(20) null comment'休息时间',
`work_day` datetime null comment '记录日期',
`delete_flg` char(1) not null comment '削除フラグ',
`create_time` datetime null comment '登録日時',
`create_user_id` varchar(20) null comment '登録者',
`update_time` datetime null comment '更新日時',
`update_user_id` varchar(20) null comment '更新者',
primary key (`id`)) comment='記録号';
